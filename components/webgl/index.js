...
export function Arm() {
  const { scene: arm1 } = useGLTF('/models/rocks4.glb')
  const { scene: arm2 } = useGLTF('/models/rocks4.glb')
...
export function WebGL({ render = true }) {
  ...
  return (
    <Canvas
      ...
      frameloop="never"
      orthographic
      camera={{ near: 0.01, far: 100000, position: [0, 0, 1000], zoom: 1 }}
    >
      <Raf render={render} />
      <Suspense>
        <Content />
